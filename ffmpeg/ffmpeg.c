...
//		T("vcodec/mpeg,vcodec/mpg1")},
	...
	{FOURCC('F','A','V','C'),CODEC_ID_H264,T("FFmpeg AVC"),
		T("vcodec/avc1,vcodec/h264,vcodec/x264,vcodec/vssh,vcodec/x-flv")},
...
static bool_t BuildOutputFormat(ffmpeg_video* p)
{
	...
	case PIX_FMT_PAL8:
		...
		break;
	case PIX_FMT_YUVJ420P: //Mod2010
		p->Codec.Out.Format.Format.Video.Pixel.Flags = PF_YUV420; //Mod2010
		break; //Mod2010
	default:
		return 0;
	}
...
static int UpdateInput( ffmpeg_video* p )
{
	...
	if (p->Codec.In.Format.Type == PACKET_VIDEO)
	{
		...
		p->Context->bits_per_coded_sample = p->Codec.In.Format.Format.Video.Pixel.BitCount;//Mod2010
