...
{
	format_reader* Reader = ((flac *)client_data)->Format.Reader;
	return Reader->Seek(Reader,(filepos_t)absolute_byte_offset,SEEK_SET) >= 0 ? FLAC__STREAM_DECODER_SEEK_STATUS_OK : FLAC__STREAM_DECODER_SEEK_STATUS_ERROR;
}
...
static int Init(flac* p)
{
	...
	state = FLAC__stream_decoder_init_stream(p->Decoder, 
		ReadCallback, 
		SeekCallback, 
		TellCallback, 
		LengthCallback, 
...
*/
	...
	if (FLAC__stream_decoder_process_until_end_of_metadata(p->Decoder) != true)
...
static int Seek(flac* p, tick_t Time, int FilePos, bool_t PrevKey)
{
	...
	if (!FLAC__stream_decoder_seek_absolute(p->Decoder,Samples))
		return ERR_NOT_SUPPORTED;
...
static int Process(flac* p,format_stream* Stream)
{
    ...
    if (FLAC__stream_decoder_process_single(p->Decoder) != true) 
	{
		FLAC__StreamDecoderState state;
		// Check state
		state = FLAC__stream_decoder_get_state(p->Decoder);
		switch (state) 
		{
