...
static bool_t Proc(playlistwin* p,int Msg, uint32_t wParam, uint32_t lParam, int* Result)
{
	...
	case WM_CREATE:
		...
		SendMessage(p->WndList,WM_SETFONT,(WPARAM)WinFont(&p->Win,&FontSize,0),0); //Mod2010
		ListViewHeader = SendMessage(p->WndList, LVM_GETHEADER, 0, 0); //Mod2010
		FontSize = 11; //Mod2010
		SendMessage(ListViewHeader,WM_SETFONT,(WPARAM)WinFont(&p->Win,&FontSize,0),0); //Mod2010
		ListView_SetExtendedListViewStyle(p->WndList,LVS_EX_FULLROWSELECT|LVS_EX_GRIDLINES); //Mod2010
