...
typedef struct asap 
{
	...
	int SamplePerBuffer;
	void* Buffer;
	//ASAP_ModuleInfo *ModuleInfo;
	ASAP_State asapState;
} asap;
...
static bool_t Load(asap* p)
{
	...
	//p->ModuleInfo = ASAP_Load(Path8, buf, n);
	if (!ASAP_Load(&p->asapState, Path8, buf, n))
		return 0;
	free(buf);
	if (!n)
		return 0;
	//ASAP_PlaySong(p->ModuleInfo->default_song, p->ModuleInfo->durations[p->ModuleInfo->default_song]);
	ASAP_PlaySong(&p->asapState, p->asapState.module_info.default_song, p->asapState.module_info.durations[p->asapState.module_info.default_song]);//p->asapState->current_duration/*p->asapState->module_info.durations[p->asapState->module_info.default_song]*/);
	p->Samples = 0;
	return 1;
}
...
static int Init(asap* p)
{
	...
	s->Format.Format.Audio.Channels = p->asapState.module_info.channels;//p->ModuleInfo->channels;//ASAP_GetChannels();
...
static int Process(asap* p,format_stream* Stream)
{
	...
	ASAP_Generate(&p->asapState, p->Buffer, BUFFER_SIZE, ASAP_FORMAT_S16_LE);
