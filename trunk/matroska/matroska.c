...
static NOINLINE void VideoTrack(format_stream* s,TrackInfo* Info,int FourCC)
{
	...
	s->Format.Format.Video.Width = Info->AV.Video.PixelWidth;   //Mod2010:
	s->Format.Format.Video.Height = Info->AV.Video.PixelHeight; //Mod2010:
		...
		if (s)
		{
			...
			if (Info)
			{
				...
				else if (strstr(Info->CodecID, "A_AAC")!=NULL)
				{
					...
					if (!CodecPrivateSize)
						BuildConfig(s,Info->CodecID,mkv_TruncFloat(Info->AV.Audio.OutputSamplingFreq)); //Mod2010:
				}
...
static int Create(matroska* p)
{
	...
	p->Format.ReadPacket = (fmtreadpacket) ReadPacket;
  	p->IO.read = Read;
	p->IO.scan = Scan;
	p->IO.getcachesize = GetSize; //Mod2010:
