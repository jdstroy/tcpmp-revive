...
{
	RECT r;
	skin tmp;//Mod2010: needed for swaping skins
	...
	if (Rotated)
	{
		...
		/*
		Mod2010: When screen is rotated swap between landscape and portrait skins
		this is the easeast way to do it :)
		*/
		if (p->Skin[0].Valid && p->Skin[1].Valid)
		{
			tmp = p->Skin[0];
			p->Skin[0] = p->Skin[1];
			p->Skin[1] = tmp;
	...
	if (!p->Win.FullScreen)
	{
...
#endif
		{
			...
			/*
			Mod2010: If aplication started in landscape mode make sure that proper
			orientation skin is displayed
			*/
			if (p->Skin[0].Valid && p->Skin[1].Valid)
				if ((r.right != p->Skin[0].Bitmap[0].Width) && (r.bottom != p->Skin[0].Bitmap[0].Height))
					if ((r.right == p->Skin[1].Bitmap[0].Width) && (r.bottom == p->Skin[1].Bitmap[0].Height))
					{
						tmp = p->Skin[0];
						p->Skin[0] = p->Skin[1];
						p->Skin[1] = tmp;
...
static void RefreshButton(intface* p,int No,bool_t* State,int Id,int Off,int On)
{
	...
	case PLAYER_SHUFFLE: i = SKIN_SHUFFLE; break; //Mod2010
	default: i = -1; break;
	}
...
//#if !defined(CONFIG_SKIN) //Mod2010:
...
static bool_t Proc(intface* p, int Msg, uint32_t wParam, uint32_t lParam, int* Result)
{
	...
	case WM_PAINT:
		...
		p->Player->Set(p->Player, PLAYER_UPDATEVIDEO, NULL, 0); //Mod2010: updates video after painting
		break;
